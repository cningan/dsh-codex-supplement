...
/**
 * 出厂质量档。取官方默认 `auto`：由服务端按提示词自行判断——既不无脑拉满
 * （更慢），也不把用户锁死在低档。要约束时延/成本就在设置里显式选一档。
 *
 * 技术债（有意未处理，用户 2026-09-24 问过）：缺省 quality 是**全局单值**，而各模型
 * 支持的**上限不同**（`1`/`1.5`/`2` 到 `high`，只有 `2.5` 到 `xhigh`/`max`）。若缺省档
 * 设为高档而清单含旧代模型，该组合结局未实测。**不做静默滑档**（用户会误以为 max 生效）、
 * 也**不做本地硬拦**（旧代模型对超限档的行为未取证，硬拦可能误伤能跑的组合）。
 * 未来正规解见工作区 `_projects/插件适配/2026-09-24-Codex能力缺口补齐.md` §八。
 */
