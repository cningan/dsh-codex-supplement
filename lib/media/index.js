...
import { DEFAULT_IMAGE_MODEL, DEFAULT_IMAGE_MODELS, IMAGE_MODEL_CHOICES } from './image-models.mjs'
import { resolveDefaultImageModel } from './model-selection.mjs'
import {
...
export const Config = z.object({
  ...
  /** 允许使用的图像模型（设置页勾选）。空数组 = 不暴露生图工具。 */
  imageModels: z.array(z.string()).default([...DEFAULT_IMAGE_MODELS]).volatile(),
  /** 工具不传 `model` 时使用的档位。 */
  defaultImageModel: z.string().default(DEFAULT_IMAGE_MODEL).volatile(),
  /** 生成结果另存目录（相对会话工作区）。 */
...
function snapshotConfig(config, ctx) {
  ...
  return {
    ...
    defaultImageModel: read('defaultImageModel', DEFAULT_IMAGE_MODEL),
...
export function apply(ctx, config) {
  ...
  const reconcileImage = () => {
    ...
    const signature = JSON.stringify([enabled, models, cfg.defaultImageModel])
    ...
    const modelSelection = {
      models,
      defaultModel: resolveDefaultImageModel({ defaultModel: cfg.defaultImageModel, selectedModels: models }),
    }
    try {
      ...
      const unregisterCapabilities = webCtx.webServer.register({
        ...
        handler: async (request, response) => {
          ...
          try {
            ...
            const cfg = current()
            const models = selectedImageModels(current)
            const defaultModel = resolveDefaultImageModel({
              defaultModel: cfg.defaultImageModel,
