...
export function imageToolDefinition(ctx, { current, name, modelSelection } = {}) {
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // 质量档：工具参数优先，其次设置页的缺省档（出厂 auto，交给服务端按提示词判断）。
      const quality = typeof args.quality === 'string' && args.quality.length > 0
        ? args.quality
      ...
      return {
        ...
        summary: `Generated ${files.length} image${files.length > 1 ? 's' : ''} with ${imageModel} at quality ${quality ?? 'auto'} via Codex subscription; saved to ${files.map((file) => file.path).join(', ')}`,
      }
    },
    presentCall: (args) => ({
      card: 'generic',
