...
import { DEFAULT_IMAGE_QUALITY, IMAGE_ACTIONS, IMAGE_BACKGROUNDS, IMAGE_QUALITIES, IMAGE_SIZES } from './adapters/openai-codex.js'
 ...
 * @param {string} [input.defaultQuality] 不传 `quality` 时使用的质量档。
 * @returns {Record<string, object>} 参数名 → schema。
 */
export function buildImageToolParameters({ models, defaultModel, defaultQuality } = {}) {
  ...
  const effectiveQuality = typeof defaultQuality === 'string' && defaultQuality.length > 0 ? defaultQuality : DEFAULT_IMAGE_QUALITY
  // 枚举只在真的有勾选时给出；一个都没勾时工具本就不会注册，这里只是不让 schema 变成空枚举。
  const modelParameter = {
    type: 'string',
    ...(enabled.length > 0 ? { enum: [...enabled] } : {}),
    description: `Image model to use.${enabled.length > 0 ? ` One of: ${enabled.join(', ')}.` : ''} Omit to use the first enabled model (${effectiveDefault}).`,
  }
  return {
    prompt: {
      type: 'string',
    ...
    quality: {
      ...
      description: `Rendering quality. GPT Image 1 / 1.5 / 2 cap at high; GPT Image 2.5 (flare/sunburst) also supports xhigh and max. Default: ${effectiveQuality}.`,
    },
    background: {
      type: 'string',
...
export function buildImageToolDescription({ models, defaultModel, outputNote } = {}) {
  ...
  const modelNote = enabled.length > 0
    ? ` Enabled image models: ${describeImageModels(enabled)}. Choose one explicitly; if omitted, the first enabled model (${effectiveDefault}) is used.`
    : ''
