...
export function normalizeOAuthSpec(input = {}) {
  ...
  const spec = {
    ...
    models: {
      mode: model.mode === 'remote' ? 'remote' : 'registry',
      api: String(model.api ?? input.accessPoint?.api ?? 'openai-completions'),
      static: Array.isArray(model.static) ? model.static.map((entry) => (typeof entry === 'string' ? { id: entry, name: entry } : { id: String(entry?.id ?? ''), name: String(entry?.name ?? entry?.id ?? '') })) : [],
    },
    // 可选：per-provider 附加头（模型发现用）；缺省 Bearer。
 ...
 *   discoverModels（委托 model-discovery.mjs）。
 *
 * 各 provider 模块 = 基座 + 自己的 spec +（可选）自定义 importCredentials。
 * @param {object} deps - { spec, accountStore, secretStore, http?, modelDiscovery? }
 */
export function createOAuthProviderBase({ spec: rawSpec, accountStore, secretStore, http, modelDiscovery }) {
  ...
  const controller = {
    ...
    /** 登录后用 token 动态 GET /models（或静态兜底），委托 model-discovery.mjs。 */
    async discoverModels({ force = false } = {}) {
      const discover = modelDiscovery ?? (async () => ({ models: [], source: 'none', diagnostics: [] }))
      return discover({ force, account: this.accountStore.select() })
    },
  }
