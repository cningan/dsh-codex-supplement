...
打开 **设置 → Codex 订阅**，即可登录、启用 Codex 搜索、查看用量/额度并配置订阅生图。在生图区勾选允许使用的图像模型：勾选决定工具 `model` 参数的取值与默认档，一个都不勾则不注册生图工具。普通对话模型及其输入/输出能力只在官方 **Models → OpenAI Codex** 项中管理。在 Codex 对话中，输入框工具栏提供额度入口与 Fast Mode 开关。
