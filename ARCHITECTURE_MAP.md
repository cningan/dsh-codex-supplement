...
| Image model selection | One surface: the configured `imageModels` list decides what is allowed and whether the tool exists at all; its **order is priority**, so the first entry is what a call that omits `model` resolves to; the tool's own `model` argument picks the tier per call | `lib/media/model-selection.mjs` | Active |
| Image quality default | One request-level tier (`imageQuality`, factory `auto`) used when a call omits `quality`; a call can still override it. Quality is a request parameter, not a model property, so it is deliberately **not** per-model — what varies per model is the ceiling (1 / 1.5 / 2 stop at `high`; only 2.5 reaches `xhigh`/`max`), and the parameter description states that | `lib/media/index.js`, `lib/media/image-tool-schema.mjs` | Active |
...
| Image-model list UI | Row-per-model editor — type an id, remove a row, add a row, or load the built-in catalog; each edit writes `imageModels` in one `settings.mutate`. No per-model explanation and no separate default selector: the description and enum already disclose every allowed model | `src/client/media.js` | Active source |
...
- Image-model selection replaced the single `codex-managed-image` sentinel. The legacy `enableProviders`/`providerModelSelections` fields are still **read once for migration only** (`lib/media/model-selection.mjs` distinguishes "explicitly disabled" from "never configured"); the legacy sentinel is never forwarded upstream. The former `defaultImageModel` field is gone — list order carries the fallback. Chat model lists are configured only in the official Models → OpenAI Codex entry.
- **Verified on a live subscription (2026-09-24):** the Codex subscription endpoint does accept `image_generation.model` — `gpt-image-2.5-flare` and `gpt-image-2.5-sunburst` each produced a PNG through the tool. The endpoint still publishes no schema (the official contract comes from the API-key SDK), so a future tightening remains possible: a rejection is surfaced verbatim (status + id) and the plugin never silently substitutes another model.
